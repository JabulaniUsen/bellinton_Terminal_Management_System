...
const ViewAgent = () => {
  ...
  const initialData = [
    { agentId: '1001', agentName: 'ABC Shipping Co.', contactPerson: 'John Smith', email: 'john@abcshipping.com', phone: '+1 (555) 123-4567', address: '123 Main Street'},
    { agentId: '1002', agentName: 'XYZ Logistics', contactPerson: 'Emily Dao', email: 'john@abcshipping.com', phone: '+1 (555) 123-4567', address: '123 Main Street'},
  ];
        ...
        <div className="flex justify-between items-center">
          <div className="">
            <div className="flex gap-2 my-10 mx-7 items-center">
              ...
              <div className="">
                  <Select
                    options={initialData.map((item) => ({ value: item.agentName, label: item.agentName }))}
              ...
              <div className="flex gap-3 justify-center items-center my-10">
                ...
                <button className=' text-white bg-[#4000FF] rounded-xl py-2 px-10' onClick={handleAddAgent} >Add Agent</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <table className="border border-collapse">
            <thead>
              <tr className="grid grid-cols-6 border border-black">
                <th className=" border bg-black text-white py-2">Agent ID</th>
                <th className=" border bg-black text-white py-2">Agent Name</th>
            ...
            <tbody>
              {data.map((rowData, index) => (
                <tr key={index} className="grid grid-cols-6">
                  <td className="border border-black px-4 py-2">{rowData.agentId}</td>
                  <td className="border border-black px-4 py-2">{rowData.agentName}</td>
